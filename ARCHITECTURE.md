...
- **Real-time collaboration** — WebSocket layer with operational transform or CRDT (Yjs) for simultaneous editing. TipTap has a collaboration extension that integrates with Yjs.
- **Document version history** — Store snapshots on save, let users view and restore previous versions.
- **Export** — PDF and Markdown export using the existing HTML content.
